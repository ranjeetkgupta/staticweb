...
import org.cloudfoundry.identity.uaa.user.CustomAuthority;
...
public class ExternalLoginAuthenticationManager implements AuthenticationManager, ApplicationEventPublisherAware, BeanNameAware {
    ...
    protected UaaUser getUser(UserDetails details, Map<String, String> info) {
        ...
        List<? extends GrantedAuthority> authorities = UaaAuthority.USER_AUTHORITIES;
        if (details instanceof ExtendedLdapUserImpl)
        {
        	
        	ExtendedLdapUserImpl extendedLdapUserImpl = (ExtendedLdapUserImpl) details;
        	String[] role = extendedLdapUserImpl.getAttributes().get("ou");
        	if(role !=null && role.length > 0)
        	{
        		String auth = role[0];
        		CustomAuthority authority = new CustomAuthority(auth);
        		authorities = CustomAuthority.getAuthorityList(authority);
